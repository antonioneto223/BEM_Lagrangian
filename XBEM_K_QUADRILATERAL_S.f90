	SUBROUTINE XBEM_K_QUADRILATERAL_S(NODE,ELEM,DOMAIN,P_ENR,N)
!
...
!
	IMPLICIT NONE
!
	INTEGER::NODE,ELEM,DOMAIN,I,J,K,KK,L,II,NEN,N,N_CRACKTIP_PAIR,POS_CT_PAIR,QSI_REF,N_GAUSS,&
    POS_CT(N_K),POS_ELEM,ELEM_CRACKTIP,NEN_CT_ELEM,ELEM_CT_S,NEN_CT_S,NEN_TEST
!
	REAL*8::PI,X,Y,Z,DX,DY,DZ,R,DRDN,VJC(3),DVJC(3,2),JC,Mu,Nu,C1,C2,AUX1,AUX2,AUX3,AUX4,DR(3),&
	ETA(3),KR(3,3),PHI[ALLOCATABLE](:),DPHI[ALLOCATABLE](:,:),D2PHI[ALLOCATABLE](:,:,:),&
    VALUES[ALLOCATABLE](:,:),COEFFICIENTS[ALLOCATABLE](:,:),&
    P_ENR(3*N_K,3),Z_FUN(3,3),Z_AUX(3,3),P_ENR_AUX(3,3),COORDS_TIP(3),VALUES_K_ALIGN(N_K,3)
	REAL*8::QSIW_GAUSS[ALLOCATABLE](:,:),QSI1D_CT,QSI1D_AL,COEFFICIENTS_K_ALIGN(N_K)
    ...
    DR_DQSI1_CT(3),DR_DQSI2_CT(3),DP_ENR(3,3),QSI1D_AL_CP(N_K),COORDS_CP_PROJECTION(3),SIGNAL,QSI_PNT(2),VALUES_ELEM_CRACKTIP[ALLOCATABLE](:,:)
    REAL*8::VALUES_CT_S[ALLOCATABLE](:,:),VALUES_TEST[ALLOCATABLE](:,:),X_CT_S,Y_CT_S,Z_CT_S,X_TEST,Y_TEST,Z_TEST,DIST_AUX(N_K)
!
    PI=DACOS(-1.D0)
...
!
            AUX1=0.D0
            AUX1=(1.D0/(R*R))*C2*QSIW_GAUSS(I,2)*QSIW_GAUSS(J,2)*JC
		    DO K=1,3
			    DO L=1,3
				    AUX2=0.D0
...
!           WILLIAMS DISPL FUNCTIONS OF EACH COLLOCATION POINT ALIGNED TO FIELD POINT
            DO K=1,N_K
                ...
                THETA_TIP_COLLOC=SIGNAL*DACOS((ROTATION_MATRIX(1,1)*DX+ROTATION_MATRIX(2,1)*DY+ROTATION_MATRIX(3,1)*DZ)/R_TIP_COLLOC)       
...
!           WILLIAMS FUNCTIONS OF FIELD POINT AND CRACK TIP
            ...
            THETA_TIP=SIGNAL*DACOS((ROTATION_MATRIX(1,1)*DX+ROTATION_MATRIX(2,1)*DY+ROTATION_MATRIX(3,1)*DZ)/R_TIP)
