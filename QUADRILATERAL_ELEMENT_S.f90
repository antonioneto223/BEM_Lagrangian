	SUBROUTINE QUADRILATERAL_ELEMENT_S(NODE,ELEM,DOMAIN,DG,DH,N)
!
...
!
	...
	DHAUX1(3,3*(ELEM_TYPE(ELEM)*ORDER_ELEM(ELEM)+(ELEM_TYPE(ELEM)-3)*(ORDER_ELEM(ELEM)-1)*POL_FAMILY(ELEM))),DPHI[ALLOCATABLE](:,:),D2PHI[ALLOCATABLE](:,:,:),MATPHI[ALLOCATABLE](:,:),VALUES[ALLOCATABLE](:,:),COEFFICIENTS[ALLOCATABLE](:,:)
...
!
            AUX_CTE1=(1.D0/R)*C1*QSIW_GAUSS(I,2)*QSIW_GAUSS(J,2)*JC
            AUX_CTE2=(1.D0/(R*R))*C2*QSIW_GAUSS(I,2)*QSIW_GAUSS(J,2)*JC
		    DO K=1,3
			    DO L=1,3
				    AUX1=0.D0
