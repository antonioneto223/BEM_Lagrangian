	SUBROUTINE WRITE_TRIMMEDOUTERFILE(PATCH,N_CURVEPOINTS,TRIMMING_CURVEPOINTS,SMART_SIZE)  
!
	USE NURBS_SURFACES
	USE TRIMMING_CURVES
!
    ...
    DO I=1,N_TRIMMINGCURVES
        IF(SURFACES(I).EQ.PATCH)THEN
            DO J=1,SUBDIVISION(I)
                CONT1=CONT1+1
                CONT2=CONT2+1
                WRITE(13,30)'K,',CONT2,',',TRIMMING_CURVEPOINTS(CONT1,1),',',TRIMMING_CURVEPOINTS(CONT1,2),',,'
            ENDDO
            CONT1=CONT1+1                   
        ENDIF
    ENDDO 
        ...
        IF(SURFACES(I).EQ.PATCH)THEN
            IF(OPENED_CLOSED_CURVE(I).EQ."O".AND.LAST_OPENED_CURVE(I).EQ."N")THEN           
                DO J=1,SUBDIVISION(I)
                    CONT1=CONT1+1
                    CONT2=CONT2+1
                    WRITE(13,40)'LSTR,     ',CONT1,',     ',CONT1+1       
                ENDDO                   
            ELSE IF(OPENED_CLOSED_CURVE(I).EQ."O".AND.LAST_OPENED_CURVE(I).EQ."S")THEN                 
                DO J=1,SUBDIVISION(I)
                    ...
                    IF(J.LT.SUBDIVISION(I))THEN
                        WRITE(13,40)'LSTR,     ',CONT1,',     ',CONT1+1
                    ELSE
                        WRITE(13,40)'LSTR,     ',CONT1,',     ',CONT1+1-CONT2
                    ENDIF
                ENDDO    
                CONT2=0                                         
