...
!        
!------------------------------------------------------------------------------------------------------------------------------------------------------
!   DEFINING THE TYPE OF COLLOCATION POINT (0=NOT CRACK TIP, 1=CRACK TIP, 2=BOUNDARY CRACK INTERSECTION) 
!------------------------------------------------------------------------------------------------------------------------------------------------------ 
!   
    DO I=1,N_ELEM   
        IF(CRACKED_ELEM(I).NE."UNCRACKED")THEN   
            ...
            DO J=1,N_COLLOCPOINTS            
                IF(CRACK_TIP(J).EQ.1)THEN
                    DO K=1,N_ELEM
                        NEN=ELEM_TYPE(K)*ORDER_ELEM(K)+(ELEM_TYPE(K)-3)*(ORDER_ELEM(K)-1)*POL_FAMILY(K)
                        DO II=1,NEN
                            IF(COLLOCPOINTS_CONNECTIVITY(K,II).EQ.J)THEN
                                P=COORD_NODES(NODES_CONNECTIVITY(K,II),:)
...
!        
!------------------------------------------------------------------------------------------------------------------------------------------------------
!   DEFINING THE TYPE OF COLLOCATION POINT (0=NOT CRACK TIP, 1=CRACK TIP, 2=BOUNDARY CRACK INTERSECTION, 3=BOUNDARY CRACK TIP) 
!------------------------------------------------------------------------------------------------------------------------------------------------------ 
!         
    DO I=1,N_ELEM
        IF(DUAL_BEM(COLLOCPOINTS_CONNECTIVITY(I,1)).NE."B")THEN
            IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,1)).EQ.2)THEN
                IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,2)).EQ.1)THEN
                    CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,1))=3
                ENDIF
                IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,4)).EQ.1)THEN
                    CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,1))=3                
                ENDIF                
            ENDIF
!
            IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,2)).EQ.2)THEN
                IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,1)).EQ.1)THEN
                    CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,2))=3
                ENDIF
                IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,3)).EQ.1)THEN
                    CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,2))=3                
                ENDIF             
            ENDIF 
!            
            IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,3)).EQ.2)THEN
                IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,2)).EQ.1)THEN
                    CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,3))=3
                ENDIF
                IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,4)).EQ.1)THEN
                    CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,3))=3                
                ENDIF             
            ENDIF    
!            
            IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,4)).EQ.2)THEN
                IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,3)).EQ.1)THEN
                    CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,4))=3
                ENDIF
                IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,1)).EQ.1)THEN
                    CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(I,4))=3                
                ENDIF             
...
!        
!------------------------------------------------------------------------------------------------------------------------------------------------------
!   DEFINING THE COORDINATES FOR THE NEW ELEMENTS AT THE BOUNDARY CRACKTIP  
!------------------------------------------------------------------------------------------------------------------------------------------------------ 
! 
    DO I=1,N_ELEM   
        IF(CRACKED_ELEM(I).EQ."CRACKTIP")THEN   
            ...
            DO J=1,N_ELEM
                IF(DUAL_BEM(COLLOCPOINTS_CONNECTIVITY(J,1)).EQ."S")THEN
                    ...
                    DO K=1,NEN
                        IF(CRACK_TIP(COLLOCPOINTS_CONNECTIVITY(J,K)).EQ.3)THEN
                                ...
                                IF(0.d0.LE.A2.AND.A2.LE.1.D0)THEN
                                    IF(A1+A2.LE.1.D0)THEN
                                        ...
                                        ELSE
                                            ...
                                            IF(R/(L).LE.BOUNDARYCRACK_TOL)THEN
                                                COORD_BOUNDARYCRACKTIP(I,:)=P 
                                                CRACKTIPQSI(1)=-1+2*A2	         
                                                CRACKTIPQSI(2)=-1+2*A1                                            
                                            ENDIF                                        
                                        ENDIF    
                                    ENDIF
                                ENDIF
